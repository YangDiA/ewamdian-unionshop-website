...
import com.ewandian.unionshop.microservice.website.websitecore.domain.service.IThemeVersionService;
 ...
 */
@RestController
@RequestMapping("/v1/themeversion")
public class ThemeVersionController {
    private static final Logger logger = LoggerFactory.getLogger(ThemeVersionController.class);

    @Autowired
    DiscoveryClient client;

    @Autowired
    private IThemeVersionService themeVersionService;

    @RequestMapping(value = "/select",method = RequestMethod.POST)
    public Result select(@RequestBody ThemeVersionDTO themeVersionDTO, HttpServletRequest request) {
        Result result = new Result();

        try {
            result.setStatus(Result.Status.OK);
            result.setData(themeVersionService.getPageEntity(themeVersionDTO));
            return result;
        } catch (Exception ex) {
    ...
    public Result add(@RequestBody  ThemeVersionDTO themeVersionDTO, HttpServletRequest request) {
        try {
            themeVersionService.saveThemeVersion(themeVersionDTO);
    ...
    @ResponseBody
    public Result update(@RequestBody  ThemeVersionDTO themeVersionDTO, HttpServletRequest request) {
        ThemeVersion themeVersion = new ThemeVersion();
        BeanUtils.copyProperties(themeVersionDTO, themeVersion);
        try {
            themeVersionService.update(themeVersion);
    ...
    public Object delete(@RequestBody String [] ids) {
        ...
        for (String i:ids){
            themeVersionService.deleteById(i);
        }
